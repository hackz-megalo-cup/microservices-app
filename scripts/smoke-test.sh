...
kubectl -n microservices rollout status deploy/caller-service --timeout=180s >/dev/null
kubectl -n microservices rollout status deploy/greeter-service --timeout=180s >/dev/null
...
kubectl -n microservices port-forward svc/greeter-service 18080:80 >/tmp/pf-greeter.log 2>&1 &
PF1=$!
...
echo "==> gRPC checks"
grpcurl -plaintext -d '{"name":"Smoke"}' 127.0.0.1:18080 greeter.v1.GreeterService/Greet >/dev/null
