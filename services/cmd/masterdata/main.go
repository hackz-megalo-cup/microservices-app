...
import (
	...
	"github.com/hackz-megalo-cup/microservices-app/services/internal/masterdata"
	"github.com/hackz-megalo-cup/microservices-app/services/internal/platform"
)

const (
	serviceName    = "masterdata-service"
...
func run() error {
	...
	// --- Database ---
	migrationsFS, _ := fs.Sub(masterdata.MigrationsFS, "migrations")
	...
	compensation.Handle(platform.TopicMasterdataFailed, func(ctx context.Context, event platform.Event) error {
		if eventStore == nil {
			return nil
		}
		...
		agg := masterdata.NewMasterdataAggregate(streamID)
		if err := platform.LoadAggregate(ctx, eventStore, agg); err != nil {
			return err
		}
		agg.Compensate("saga compensation")
		if err := platform.SaveAggregate(ctx, eventStore, outbox, agg, masterdata.MasterdataTopicMapper); err != nil {
			return err
		}
	...
	go func() {
		if err := compensation.Run(ctx, brokers, "masterdata-compensation"); err != nil && !errors.Is(err, context.Canceled) {
	...
	// --- Service ---
	svc := masterdata.NewService(eventStore, outbox)
	...
	path, handler := masterdatav1connect.NewMasterdataServiceHandler(
		svc,
