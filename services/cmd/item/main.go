...
func run() error {
	...
	// --- Database ---
	migrationsFS, _ := fs.Sub(item.MigrationsFS, "migrations")
	...
	compensation.Handle(platform.TopicItemFailed, func(ctx context.Context, event platform.Event) error {
		if eventStore == nil {
			return nil
		}
		...
		agg := item.NewItemAggregate(streamID)
		if err := platform.LoadAggregate(ctx, eventStore, agg); err != nil {
			return err
		}
		agg.Compensate("saga compensation")
		if err := platform.SaveAggregate(ctx, eventStore, outbox, agg, item.ItemTopicMapper); err != nil {
			return err
		}
	...
	go func() {
		if err := compensation.Run(ctx, brokers, "item-compensation"); err != nil && !errors.Is(err, context.Canceled) {
	...
	path, handler := itemv1connect.NewItemServiceHandler(
		svc,
