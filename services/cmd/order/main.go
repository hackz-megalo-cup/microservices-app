...
import (
	...
	"github.com/hackz-megalo-cup/microservices-app/services/gen/go/order/v1/orderv1connect"
	"github.com/hackz-megalo-cup/microservices-app/services/internal/order"
...
func run() error {
	...
	// --- Database ---
	migrationsFS, _ := fs.Sub(order.MigrationsFS, "migrations")
	...
	compensation.Handle(platform.TopicOrderFailed, func(ctx context.Context, event platform.Event) error {
		if eventStore == nil {
			return nil
		}
		...
		agg := order.NewOrderAggregate(streamID)
		if err := platform.LoadAggregate(ctx, eventStore, agg); err != nil {
			return err
		}
		agg.Compensate("saga compensation")
		if err := platform.SaveAggregate(ctx, eventStore, outbox, agg, order.OrderTopicMapper); err != nil {
			return err
		}
	...
	go func() {
		if err := compensation.Run(ctx, brokers, "order-compensation"); err != nil && !errors.Is(err, context.Canceled) {
	...
	// --- Service ---
	svc := order.NewService(eventStore, outbox)
