...
const (
	EventCreated = "masterdata.created"
	// ↓ ドメインイベントを追加する
	// 例: EventCompleted = "masterdata.completed"
	// ⚠ 新しいイベントを追加したら platform/topics.go にもトピック定数と DefaultTopics() を追加すること。

	EventFailed      = "masterdata.failed"      // main.go が参照 — 削除禁止
	EventCompensated = "masterdata.compensated" // main.go が参照 — 削除禁止
)
